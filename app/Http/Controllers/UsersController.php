...
class UsersController extends Controller
{
    ...
    public function UpdateUser(Request $request)
    {
        $this->validate($request,[
            ...
            'edit_phone'    =>'required|min:2|max:190|unique:users,phone,'.$request->id,
            'edit_avatar'   =>'nullable|image|mimes:jpeg,png,jpg,gif,svg',
            'edit_password' =>'nullable|min:6|max:190'
        ],[
            'edit_email.unique'=> ' قيمة الايميل مستخدمة من قبل',
            'edit_phone.unique'=> ' قيمة الهاتف مستخدمة من قبل'
        ]);
        ...
        if($request->active != "")
        {
            if($user->id != 1 and Auth::id()!=$user->id)
            {
                $user->active     =$request->active;
            }else
            {
